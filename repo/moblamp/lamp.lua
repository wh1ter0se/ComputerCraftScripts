...
function On()
    for i = 1,#Sides do redstone.setOutput(Sides[i], true) end
    print('On')
end

function Off()
    for i = 1,#Sides do redstone.setOutput(Sides[i], false) end
    print('Off')
end

function lamp.Run(lampID)
        -- Find modems
    local tModems = {}
                ...
                if nChannel == rednet.CHANNEL_REPEAT then
                    if type(tMessage) == "table" and tMessage.nMessageID and tMessage.nRecipient and type(tMessage.nRecipient) == "number" then
                        if not tReceivedMessages[tMessage.nMessageID] then
                            ...
                            -- Flip the lights
                            local msg = tMessage.message
                            for k,v = pairs(msg) do
                                if k == 'sText' then
                                    print(v)
                                    local chunks = util.split(msg,';')
                                    if string.find(chunks[1],lampID) then
                                        if string.find(chunks[2],'on') then
                                            On()
                                        elseif string.find(chunks[2],'off') then
                                            Off()
                                        end
                                    end
                                end
